...
export default function ServiceCard({ icon, title, description, features, showDetails = false }) {
  ...
  return (
    <div
      className={`group relative bg-white border border-slate-200 rounded-xl p-6 card-hover flex flex-col ${
    ...
    >
      ...
      <motion.div
        ...
        whileHover={{ scale: 1.18, rotate: 6, backgroundColor: 'rgba(0,102,204,0.18)' }}
        transition={{ type: 'spring', stiffness: 320, damping: 14 }}
      >
        <IconComponent className="w-6 h-6 text-accent" />
      </motion.div>

      <h3 className="font-syne font-bold text-lg text-slate-900 group-hover:text-accent transition-colors duration-200">
        {title}
      </h3>

      <p className={`text-slate-600 text-sm leading-relaxed ${showDetails ? '' : 'line-clamp-3'}`}>
        {description}
      </p>

      {showDetails && features && features.length > 0 && (
        <div className="space-y-2.5">
          <h4 className="font-syne font-semibold text-sm text-slate-700 uppercase tracking-wide">
            Key Features
          </h4>
      ...
      {showDetails ? (
        <div className="flex flex-col sm:flex-row gap-3 mt-auto pt-2">
          <Link
            to="/contact"
            className="flex-1 text-center bg-accent text-white font-syne font-semibold text-sm px-4 py-2.5 rounded hover:bg-accent/90 transition-all duration-200"
          >
            Get Quote
          </Link>
          <Link
