...
export default function Contact() {
  ...
  const inputClass = (field) =>
    `w-full bg-secondary border rounded-lg px-4 py-3 text-text-base text-sm placeholder-text-muted outline-none transition-all duration-200 input-focus-glow ${
      errors[field]
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-0">
            {trustSignals.map(({ icon: Icon, label, sub }) => (
              <div key={label} className="flex items-start gap-3 bg-surface border border-border rounded-xl p-4">
                <div className="w-9 h-9 rounded-lg bg-accent-muted border border-accent-dim flex items-center justify-center shrink-0">
                  <Icon className="w-4 h-4 text-accent" />
                </div>
                <div>
                  <p className="text-text-base text-sm font-syne font-semibold">{label}</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-8">
              <div>
                <h2 className="font-syne font-bold text-2xl text-text-base mb-6">
                  Contact Information
                </h2>
                <div className="space-y-4">
                  {contactDetails.map(({ icon: Icon, label, value, link }) => (
                    <div key={label} className="flex gap-4 bg-surface border border-border rounded-xl p-5">
                      <div className="w-10 h-10 rounded-lg bg-accent-muted border border-accent-dim flex items-center justify-center shrink-0">
                        <Icon className="w-5 h-5 text-accent" />
                      </div>
                      <div>
                        <div className="text-text-muted text-xs font-medium uppercase tracking-wide mb-1">{label}</div>
                        {link ? (
                          <a href={link} className="text-text-sub text-sm hover:text-accent transition-colors">{value}</a>
                        ) : (
                          <p className="text-text-sub text-sm leading-relaxed">{value}</p>
                        )}
                      </div>
                    </div>
                  ))}
              ...
              <div className="rounded-xl overflow-hidden border border-border bg-surface shadow-card">
                <div className="h-64 flex flex-col items-center justify-center gap-4 relative">
                  ...
                  <div className="relative z-10 flex flex-col items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-accent-muted border border-accent-dim flex items-center justify-center">
                      <MapPin className="w-6 h-6 text-accent" />
                    </div>
                    <div className="text-center">
                      <p className="text-text-base font-syne font-semibold text-sm">Alites Cybersecurity Solutions</p>
                      <p className="text-text-muted text-xs mt-1">No. 12, Tech Park, Coimbatore – 641014</p>
                    </div>
                    <div className="px-4 py-2 rounded-lg text-xs text-accent border border-accent-dim bg-secondary cursor-pointer hover:bg-accent-muted transition-colors">
                      View on Google Maps
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-surface border border-border rounded-xl p-5 shadow-card">
                <h3 className="font-syne font-semibold text-text-base text-sm mb-4">Availability</h3>
                <div className="space-y-2">
                  {[
                    ...
                    <div key={day} className="flex justify-between items-center text-sm">
                      <span className="text-text-sub">{day}</span>
                      <span className="text-text-base font-medium">{hours}</span>
                    </div>
                  ))}
                </div>
                ...
                {submitted ? (
                  ...
                  >
                    <motion.div
                      ...
                      className="w-16 h-16 rounded-full bg-green-900/20 border border-green-700/40 flex items-center justify-center"
                    >
                      <CheckCircle2 className="w-8 h-8 text-green-400" />
                    </motion.div>
                    <div>
                      <h3 className="font-syne font-bold text-text-base text-2xl mb-2">
                        Message Received, {form.name.split(' ')[0]}
                      </h3>
                      <p className="text-text-sub text-sm leading-relaxed max-w-sm">
                  ...
                  <>
                    ...
                    <form onSubmit={handleSubmit} noValidate className="space-y-5">
                      <div>
                        <label htmlFor="name" className="block text-text-sub text-xs font-medium uppercase tracking-wide mb-1.5">
                      ...
                      <div>
                        <label htmlFor="email" className="block text-text-sub text-xs font-medium uppercase tracking-wide mb-1.5">
                      ...
                      <div>
                        <label htmlFor="company" className="block text-text-sub text-xs font-medium uppercase tracking-wide mb-1.5">
                      ...
                      <div>
                        <label htmlFor="service" className="block text-text-sub text-xs font-medium uppercase tracking-wide mb-1.5">
                        ...
                        <select id="service" name="service" value={form.service} onChange={handleChange} className={`${inputClass('service')} cursor-pointer`}>
                          ...
                          {serviceOptions.map((opt) => (
                            <option key={opt} value={opt} className="bg-secondary text-text-base">{opt}</option>
                          ))}
                        </select>
                      ...
                      <div>
                        <label htmlFor="message" className="block text-text-sub text-xs font-medium uppercase tracking-wide mb-1.5">
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-full flex items-center justify-center gap-2 bg-accent text-white font-syne font-bold py-3.5 px-6 rounded-lg hover:bg-accent-bright transition-all duration-200 hover:shadow-lg hover:shadow-accent-muted disabled:opacity-70 disabled:cursor-not-allowed"
                      >
                        {loading ? (
                          <>
