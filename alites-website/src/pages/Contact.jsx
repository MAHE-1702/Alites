...
export default function Contact() {
  ...
  const inputClass = (field) =>
    `w-full bg-white/5 border rounded-lg px-4 py-3 text-white text-sm placeholder-slate-500 outline-none transition-all duration-200 input-focus-glow ${
      errors[field]
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-0">
            {trustSignals.map(({ icon: Icon, label, sub }) => (
              <div key={label} className="flex items-start gap-3 bg-[#0D0D0D] border border-white/10 rounded-xl p-4">
              ...
              <div>
                ...
                <div className="space-y-4">
                  {contactDetails.map(({ icon: Icon, label, value, link }) => (
                    <div key={label} className="flex gap-4 bg-[#0D0D0D] border border-white/10 rounded-xl p-5">
              ...
              <div className="rounded-xl overflow-hidden border border-white/10 bg-white/5">
                <div className="h-64 flex flex-col items-center justify-center gap-4 relative">
                  ...
                  <div className="relative z-10 flex flex-col items-center gap-3">
                    ...
                    <div className="px-4 py-2 rounded-lg text-xs text-accent border border-accent/30 bg-[#0D0D0D] cursor-pointer hover:bg-accent/5 transition-colors">
                      View on Google Maps
                    </div>
                  </div>
                </div>
                  ...
                  <>
                    ...
                    <form onSubmit={handleSubmit} noValidate className="space-y-5">
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-full flex items-center justify-center gap-2 bg-accent text-white font-syne font-bold py-3.5 px-6 rounded-lg hover:bg-accent/90 transition-all duration-200 hover:shadow-lg hover:shadow-accent/25 disabled:opacity-70 disabled:cursor-not-allowed"
                      >
                        {loading ? (
                          <>
